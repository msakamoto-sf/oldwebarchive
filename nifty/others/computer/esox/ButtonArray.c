...
/* Draws Non 3D rectangle */
void Simple2dRect(Window wn, GC _hilite, GC _shadow, int x, int y, int w, int h)
{
  int y_offset = -2;

	  XDrawLine (dpy, wn, _hilite, x, y, x+w, y);
	  XDrawLine (dpy, wn, _shadow, x, y+h+y_offset, x+w, y+h+y_offset);

	  XDrawLine (dpy, wn, _hilite, x, y, x, y+h+y_offset);
	  XDrawLine (dpy, wn, _shadow, x+w, y, x+w, y+h+y_offset);
}
	...
	int state, Bool iconified)
{
	...
	*/
	...
	if (iconified)
	{
		...
		Simple2dRect(wn, iconhilite, iconshadow, x, y, w, h);
	...
	{
	case BUTTON_UP:
	  Simple2dRect(wn, hilite, shadow, x, y, w, h);
	  /*XDrawLine (dpy, wn, blackgc, x+w-1, y+h-1, x+w-1, y);*/
		break;
	case BUTTON_BRIGHT:
		draw = frect_get_intersection(
		...
		if (!NoBrightFocus)
			XFillRectangle (
				...
				inter.width, inter.height);
		Simple2dRect(wn, focushilite, focusshadow, x, y, w, h);
		break;
	case BUTTON_DOWN:
	  Simple2dRect(wn, hilite, hilite, x, y, w, h);
		break;
	}
}
